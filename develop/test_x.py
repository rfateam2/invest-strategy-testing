# python test_x.py QQQ 100 2015-01-01 --end_date 2024-12-31
...
def apply_test_strategy(data, weekly_investment, day_of_week):
    total_invested = 0
    total_units = 0
    max_price = 0
    portfolio_value = []
    ...
    # Загружаем данные для QLD и TQQQ
    for ticker in tickers.keys():
        try:
            ...
            raise ValueError(f"Не удалось загрузить данные для тикера {ticker}: {e}")

    # Слияние данных с основным DataFrame
...
def plot_results(data, simple_values, test_values, simple_monthly, test_monthly):
    ...
    plt.plot(data["Date"], np.interp(data["Date"], simple_monthly_totals["YearMonth"], simple_cumulative_investment),
             label="Стратегия: Вложенные средства", color="blue", linestyle="--", alpha=0.5)
...
def main():
    ...
    test_invested, test_values, test_monthly = apply_test_strategy(data, args.weekly_investment, args.day_of_week)
