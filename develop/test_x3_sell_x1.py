# python test_x3_sell_x1.py 100 --start_date 2015-01-01 --end_date 2024-12-31
...
def apply_test_strategy(data, weekly_investment):
    ...
    for current_date in pd.date_range(data["Date"].min(), data["Date"].max(), freq="W-FRI"):
        ...
        print(f"Current QQQ Price: {close:.2f}")

        if close > max_price and (shares["QLD"] > 0 or shares["TQQQ"] > 0):
            print(f"QQQ exceeded previous max price {max_price:.2f}. Rebalancing...")
            qld_value = shares["QLD"] * row.get("QLD_Close", 0)
            tqqq_value = shares["TQQQ"] * row.get("TQQQ_Close", 0)
            total_to_reinvest = qld_value + tqqq_value + weekly_investment
            print(f"Selling QLD for ${qld_value:.2f}, TQQQ for ${tqqq_value:.2f}. Total to reinvest: ${total_to_reinvest:.2f}")
            shares["QQQ"] += total_to_reinvest / close
            print(f"Buying {total_to_reinvest / close:.2f} shares of QQQ at ${close:.2f}")
            shares["QLD"] = shares["TQQQ"] = 0
            total_units = shares["QQQ"]

        if close > max_price and (shares["QLD"] > 0 or shares["TQQQ"] > 0):  # If price exceeds previous max and we have leveraged ETFs
            # Sell all QLD and TQQQ
            qld_value = shares["QLD"] * row.get("QLD_Close", 0)
            tqqq_value = shares["TQQQ"] * row.get("TQQQ_Close", 0)
            total_to_reinvest = qld_value + tqqq_value + weekly_investment
            
            # Buy QQQ with all the money from selling QLD, TQQQ, and the new weekly investment
            shares["QQQ"] += total_to_reinvest / close
            shares["QLD"] = 0
            shares["TQQQ"] = 0
            total_units = shares["QQQ"]  # Reset to QQQ units
        elif close <= max_price * 0.8:
        ...
        print(f"Portfolio: QQQ={shares['QQQ']:.2f}, QLD={shares['QLD']:.2f}, TQQQ={shares['TQQQ']:.2f}")
        print(f"Portfolio Value: ${total_units * close:.2f}")
    ...
    # Correct portfolio value calculation at the end of the period
    portfolio_value[-1] = (shares["QQQ"] * close + 
                           shares["QLD"] * row.get("QLD_Close", 0) + 
                           shares["TQQQ"] * row.get("TQQQ_Close", 0))

    return total_invested, portfolio_value, invested_amounts, dates, shares


def plot_results(simple_dates, simple_portfolio, simple_invested, test_dates, test_portfolio, test_invested):
...
def main():
    ...
    # Отображение графика
    plot_results(simple_dates, simple_portfolio, simple_invested_curve, test_dates, test_portfolio, test_invested_curve)
