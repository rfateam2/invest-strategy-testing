...
def apply_test_strategy_with_volatility(data, weekly_investment, price_step, day_of_week, multiplier=1, rolling_window=20):
    total_invested = 0
    total_units = 0
    portfolio_value = []
    monthly_investments = []
    ...
    max_price = 0  # Изначально максимальная цена равна 0

    for _, row in data.iterrows():
        date, close, day_name, rolling_std = row["Date"], row["Close"], row["DayOfWeek"], row["RollingStd"]

        # Обновляем максимальную цену
        max_price = max(max_price, close)

        # Если день недели совпадает, выполняем стандартное пополнение
        if day_name == day_of_week:
            total_units += weekly_investment / close
            total_invested += weekly_investment
            monthly_investments.append((date.strftime("%Y-%m"), weekly_investment))

        # Определяем динамические уровни покупки
        for k in [1, 2, 3]:  # Множители для порогов
            if rolling_std is not None:  # Убедимся, что std рассчитан
                dynamic_threshold = max_price * (1 - k * rolling_std / max_price)

                if close <= dynamic_threshold:
                    extra_investment = weekly_investment * price_step * multiplier
                    total_units += extra_investment / close
                    total_invested += extra_investment
                    monthly_investments.append((date.strftime("%Y-%m"), extra_investment))
                    break  # Покупка выполняется один раз за итерацию
...
def main():
    ...
    parser.add_argument("--rolling_window", type=int, default=20, help="Период для расчета скользящего стандартного отклонения.")
    ...
    # Стратегия ATR
    test_invested, test_values, test_monthly = apply_test_strategy_with_volatility(
        data, args.weekly_investment, args.price_step, args.day_of_week, args.multiplier, args.rolling_window
    )
