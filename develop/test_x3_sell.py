# python test_x3_sell.py 100 --start_date 2015-01-01 --end_date 2024-12-31
import argparse
...
def apply_test_strategy(data, weekly_investment):
    ...
    for current_date in pd.date_range(data["Date"].min(), data["Date"].max(), freq="W-FRI"):
        ...
        if close > max_price and (shares["QLD"] > 0 or shares["TQQQ"] > 0):  # If price exceeds previous max and we have leveraged ETFs
            ...
            shares["QQQ"] += total_to_reinvest / close  # Buy QQQ with all the money from QLD, TQQQ, and weekly investment
            shares["QLD"] = 0
            shares["TQQQ"] = 0
            total_units = shares["QQQ"]  # Reset total units to only QQQ
